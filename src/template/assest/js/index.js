...
const toggleList = (id) => {
  ...
  sessionStorage.setItem('activeList', JSON.stringify(Array.from(activeList)))
}

// 判断是否展开菜单
const openMenu = () => {
  const navList = sessionStorage.getItem('activeList')
    ? JSON.parse(sessionStorage.getItem('activeList'))
    : []
  if (navList.length) {
    activeList = new Set(navList)
...
export const randerMenu = (menuList, isSearch) => {
  ...
  if (!isSearch) {
    openMenu()
    let apiInfo = sessionStorage.getItem('selectApiInfo')
      ? JSON.parse(sessionStorage.getItem('selectApiInfo'))
      : {}
    if (!Object.keys(apiInfo).length) {
      apiInfo = findText(menuList)
      const id = apiInfo.id
...
const addEvent = function () {
  ...
  textList.forEach((item) => {
    const fn = () => {
      ...
      textList.forEach((option) => {
        option.classList.remove('active')
      })
      // 选中的菜单
      const selectApiInfo = menu
      sessionStorage.setItem('selectApiInfo', JSON.stringify(selectApiInfo))
